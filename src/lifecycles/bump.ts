...
export class BumpLifecycle extends Lifecycle {
    ...
    private async syncLockfileWorkspaceVersion(allFiles: BumpFile[], scanDir: string, context: CommandContext): Promise<void> {
        ...
        // Add lockfilePath to bumpFiles if not already included
        const projectRoot = context.options?.cwd || process.cwd();
        const bumpFiles = context.options.bumpFiles || [];
        const isExist = bumpFiles.some((file) => {
            const filename = typeof file === 'string' ? file : file.filename;
            return resolveFilePath(filename, projectRoot) === lockfilePath;
        });
        if (!isExist) {
            context.options.bumpFiles = [...bumpFiles, path.relative(projectRoot, lockfilePath)];
        }
    }
}
